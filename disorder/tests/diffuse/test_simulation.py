...
class test_simulation(unittest.TestCase):
    ...
    def test_structural_energy(self):
        ...
        sigma = np.zeros((nu,nv,nw,n_atm,M))

        Ux = np.zeros((nu,nv,nw,n_atm,M))
        Uy = np.zeros((nu,nv,nw,n_atm,M))
        Uz = np.zeros((nu,nv,nw,n_atm,M))

        sigma[...,0] = +1
        sigma[...,1] = +1
        sigma[...,2] = -1

        Ux[...,0], Uy[...,0], Uz[...,0] = 1, 0, 0
        Ux[...,1], Uy[...,1], Uz[...,1] = 0, 1, 0
        Ux[...,2], Uy[...,2], Uz[...,2] = 0, 0, 1

        E = simulation.structural_energy(sigma, Ux, Uy, Uz, J, H, K, eta,
                                         dx, dy, dz, d, atm_ind,
        ...
        self.assertAlmostEqual(E[...,0].sum(),
                               -(0.5*(J0-K0*(r*eta0)**2)*n_pair+H0)*n)
    ...
    def test_size_effect(self):
        ...
        sigma = 2.0*np.random.randint(0,2,(nu,nv,nw,n_atm,M))-1
        ...
        E, T_range = simulation.size_effect(sigma, Ux, Uy, Uz, J, H, K, eta,
                                            dx, dy, dz, d, atm_ind,
                                            img_i, img_j, img_k, pair_ind,
                                            T_range, kB, N)

        E_ref = simulation.structural_energy(sigma, Ux, Uy, Uz, J, H, K, eta,
                                             dx, dy, dz, d, atm_ind,
                                             img_i, img_j, img_k, pair_ind)

        E0 = E_ref.sum(axis=(0,1,2,3,4))
